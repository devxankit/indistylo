...
interface AddFeaturedServiceModalProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

export function AddFeaturedServiceModal({ open, onOpenChange }: AddFeaturedServiceModalProps) {
    ...
    // Store Data
    const { addFeaturedService } = useContentStore();
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            await addFeaturedService(newService);
            onOpenChange(false);
        } catch (error) {
            ...
            <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto p-6 bg-background border-border shadow-2xl">
                <DialogHeader className="mb-4">
                    <div className="flex items-center justify-between">
                        <DialogTitle className="text-xl font-bold">Add Featured Service</DialogTitle>
