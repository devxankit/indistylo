...
const maleCategories = [
  { id: 1, name: 'Haircut & styling', image: 'https://images.unsplash.com/photo-1599351431202-1e0f0137899a?auto=format&fit=crop&q=80&w=200' },
  { id: 2, name: 'Facials & Cleanups', image: 'https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?auto=format&fit=crop&q=80&w=200' },
  { id: 3, name: 'Shave & Beard', image: 'https://images.unsplash.com/photo-1599351431202-1e0f0137899a?auto=format&fit=crop&q=80&w=200' },
  { id: 4, name: 'Hand & Feet', image: 'https://images.unsplash.com/photo-1632345031435-8727f6897d53?auto=format&fit=crop&q=80&w=200' },
];
...
export function AtHomePage() {
  ...
  const currentCategories = selectedGender === 'male' ? maleCategories : femaleCategories;
  const currentPackages = selectedGender === 'male' ? malePackages : femalePackages;
  ...
  const handleBuyNow = (pkg: typeof currentPackages[0]) => {
    ...
    addItem(packageItem, 1, selectedGender);
  ...
  return (
    <div className="min-h-screen bg-background pb-24 text-foreground">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 sm:px-6 py-6 sm:py-0 min-h-[200px] sm:h-full">
          <div className="z-20 max-w-[65%] sm:max-w-[60%] space-y-2 pr-2">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold leading-tight text-foreground">
              Transform your look <span className="text-primary">At-Home !</span>
            </h1>
        ...
        <div className="space-y-4">
          <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
            {currentCategories.map((cat) => (
              <div key={cat.id} className="flex flex-col items-center gap-2 min-w-[100px] cursor-pointer group">
                ...
                <span className={cn("text-xs text-center font-medium", cat.id === 1 ? "text-primary" : "text-muted-foreground")}>
                  {cat.name}
                </span>
              </div>
            ))}
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-100">Grab Exciting Packages</h2>
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide">
        {currentPackages.map((pkg) => (
          <div
            key={pkg.id}
            className="flex-shrink-0 w-80 bg-card border border-gray-700 rounded-xl overflow-hidden hover:border-yellow-400/50 transition-colors"
          >
            {/* Card Content */}
            <div className="p-5 flex flex-col h-full">
              {/* Title */}
              <h3 className="text-base font-semibold text-gray-100 mb-2 text-left">{pkg.title}</h3>
              ...
              {/* Price and Button */}
              <div className="flex items-center justify-between">
                <span className="text-xl font-bold text-yellow-400">₹ {pkg.price}</span>
                <button
                  onClick={() => handleBuyNow(pkg)}
                  className="bg-transparent border border-yellow-400 text-yellow-400 flex align-center items-center justify-center rounded-lg cursor-pointer text-md hover:bg-yellow-400/20 hover:text-yellow-400 hover:border-yellow-400 font-semibold px-6 h-9 text-md transition-colors"
                >
                  Buy Now
                </button>
              </div>
