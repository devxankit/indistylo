...
import { useUserStore, type UserNotification } from "../store/useUserStore";
...
export function NotificationModal({ open, onOpenChange }: NotificationModalProps) {
    const { notifications, markAsRead, markAllAsRead, clearNotifications, deleteNotification } = useUserStore();
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[420px] p-0 overflow-hidden gap-0 bg-background/95 backdrop-blur-xl border-border/50 shadow-2xl">
                <DialogHeader className="px-5 py-4 border-b border-border bg-muted/40 flex flex-row items-center justify-between sticky top-0 z-10">
                ...
                <div className="h-[450px] w-full overflow-y-auto custom-scrollbar">
                    {notifications.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-[350px] text-center p-8 text-muted-foreground">
                        ...
                        <div className="divide-y divide-border/50">
                            {notifications.map((notification) => (
                                <div
                                    ...
                                    className={cn(
                                        "flex gap-4 p-4 transition-all hover:bg-muted/50 cursor-pointer group relative text-left",
